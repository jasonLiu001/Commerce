...
using DataAccess.MySQL.Helper;
using DataAccess.SqlServer.Helper;
...
namespace DataAccess.Helper
{
    public class Utility
    {
        ...
        public static List<T> GetListFromSqlServer<T>(string[] selectedColumns, string sql) where T : new()
        {
            List<T> list = new List<T>();           
            DataSet ds = SqlServerHelper.ExecuteDataset(SqlServerHelper.ConnStr, CommandType.Text, sql);
        ...
        public static List<T> GetListFromMySql<T>(string[] selectedColumns, string sql) where T : new()
        {
            List<T> list = new List<T>();
            DataSet ds = MySqlHelper.ExecuteDataset(SqlServerHelper.ConnStr, CommandType.Text, sql);
            DataTable dt = ds.Tables[0];
            foreach (DataRow dr in dt.Rows)
            {
                var obj = new T();
